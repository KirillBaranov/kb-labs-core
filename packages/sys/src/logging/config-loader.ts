...
import { configureLogger, type LogSink } from "./index.js";
import { configureAI } from "./ai-config.js";
import { initLogging } from "./init.js";
import { createFileSink } from "./sinks/file-sink.js";
import { createConsoleSink } from "./sinks/console-sink.js";
import { jsonSink } from "./sinks/json.js";
import { createRedactor } from "./redaction.js";
import { createEnrichmentSink } from "./adapters/enrichment-adapter.js";
import type { LogLevel, AIConfig } from "./types/types.js";
...
async function loadOptionalAdapter(type: string, config: any): Promise<LogSink | null> {
  try {
    switch (type) {
      case 'sentry':
        const sentryModule = await import('./adapters/sentry-adapter.js');
        return sentryModule.createSentryAdapter(config);
      case 'loki':
        const lokiModule = await import('./adapters/loki-adapter.js');
        return lokiModule.createLokiAdapter(config);
      case 'elasticsearch':
        const esModule = await import('./adapters/elasticsearch-adapter.js');
        return esModule.createElasticsearchAdapter(config);
      case 'datadog':
        const datadogModule = await import('./adapters/datadog-adapter.js');
