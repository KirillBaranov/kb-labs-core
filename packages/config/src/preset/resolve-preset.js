 ...
 */
export async function resolvePreset(presetRef, cwd) {
    ...
    throw new KbError('ERR_PRESET_NOT_RESOLVED', `Preset not found: ${presetRef}`, ERROR_HINTS.ERR_PRESET_NOT_RESOLVED, { presetRef, cwd });
}
/**
...
function parsePresetRef(presetRef) {
    ...
    if (atIndex === -1) {
        throw new KbError('ERR_PRESET_REF_INVALID', `Invalid preset reference: ${presetRef}`, 'Preset references must include version (e.g., @kb-labs/org-preset@1.3.2)', { presetRef });
    }
    const name = presetRef.substring(0, atIndex);
    const version = presetRef.substring(atIndex + 1);
    if (!name || !version) {
        throw new KbError('ERR_PRESET_REF_INVALID', `Invalid preset reference: ${presetRef}`, 'Preset references must include both name and version', { presetRef });
    }
    return [name, version];
}
/**
 ...
 */
export function getPresetConfigForProduct(preset, product) {
