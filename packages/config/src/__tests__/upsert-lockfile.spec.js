...
describe('upsertLockfile', () => {
    ...
    describe('lockfile creation', () => {
        it('should create new lockfile when none exists', async () => {
            ...
            expect(mockEnsureWithinWorkspace).toHaveBeenCalledWith(path.join(testDir, '.kb', 'lock.json'), testDir);
    ...
    describe('deprecated lockfile detection', () => {
        it('should warn about deprecated lockfile.json', async () => {
            ...
            await fsp.writeFile(path.join(testDir, '.kb', 'lockfile.json'), JSON.stringify({ old: 'format' }));
            mockReadLockfile.mockResolvedValue(null);
            mockWriteLockfile.mockResolvedValue(undefined);
            const result = await upsertLockfile({
                cwd: testDir,
            ...
            expect(result.warnings).toContain('Deprecated lockfile found at .kb/lockfile.json, please migrate to .kb/lock.json');
        });
    ...
    describe('workspace boundary validation', () => {
        it('should validate workspace boundaries', async () => {
            ...
            mockEnsureWithinWorkspace.mockImplementation(() => {
                throw new KbError('ERR_PATH_OUTSIDE_WORKSPACE', 'Refusing to write outside workspace', 'Check cwd or use a relative path within the workspace', { targetPath: 'outside', workspaceRoot: testDir });
            });
            await expect(upsertLockfile({
                cwd: testDir,
                dryRun: false,
    ...
    describe('path resolution', () => {
        it('should resolve relative paths correctly', async () => {
            ...
            expect(mockEnsureWithinWorkspace).toHaveBeenCalledWith(expect.stringContaining('.kb/lock.json'), expect.stringContaining('test-relative'));
        });
    });
});
//# sourceMappingURL=upsert-lockfile.spec.js.map