...
describe('runtime utils', () => {
    ...
    describe('readJsonWithDiagnostics', () => {
        ...
        it('returns diagnostics on parse error', async () => {
            ...
            if (!res.ok) {
                expect(res.diagnostics.find(d => d.code === 'JSON_PARSE_FAILED')).toBeTruthy();
            }
        });
        it('returns diagnostics on read error', async () => {
            ...
            expect(res.ok).toBe(false);
            if (!res.ok) {
                expect(res.diagnostics.find(d => d.code === 'FILE_READ_FAILED')).toBeTruthy();
            }
        });
    });
    describe('mergeDefined', () => {
        it('deep merges objects and arrays ignoring undefined', () => {
            ...
            const over = { a: undefined, b: { y: [2], z: 3 }, c: [3], d: 'set', e: 5 };
    ...
    describe('pickDefined', () => {
        it('picks only defined values', () => {
            const res = pickDefined({ a: 1, b: undefined, c: false });
    ...
    describe('resolveConfig', () => {
        it('applies precedence: defaults < file < env < cli; and validate', () => {
            ...
            const envMapper = (env) => ({ b: { x: Number(env.TEST_X ?? 1), z: Number(env.TEST_Z ?? 0) }, c: env.TEST_C });
            const cli = { c: 'cli', f: undefined };
            const validate = (cfg) => ({ ok: cfg.a > 0, diagnostics: [{ level: 'info', code: 'VALIDATED', message: 'ok' }] });
            const { value, diagnostics } = resolveConfig({ defaults, fileConfig: file, envMapper, cliOverrides: cli, validate });
        ...
        it('env mapper reads real env; can be stubbed', () => {
            ...
            const { value } = resolveConfig({ defaults: defaults, envMapper: (e) => ({ b: { z: Number(e.TEST_Z) } }) });
            expect(value).toEqual({
                b: { z: 5 },
                profiles: { rootDir: '.kb/profiles', defaultName: 'default', strict: true }
            });
